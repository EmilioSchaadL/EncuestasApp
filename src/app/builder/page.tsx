...
export default function BuilderPage() {
  ...
  const handleSaveToSheets = async () => {
    setIsSaving(true);
    const newSurveyId =
      "SURVEY-" + Math.random().toString(36).substring(2, 9).toUpperCase();
    const surveyToSave = {
      id: newSurveyId,
      title,
      description,
    ...
    try {
      const response = await saveSurveyData(
        surveyToSave,
        username || "anonymous",
      );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-xs text-neutral-500 uppercase tracking-widest">
                {username ? `Autor: ${username}` : "Modo Diseño"}
              </span>
            </div>
          </div>
        ...
        {createdUrl ? (
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-[2rem] p-12 text-center shadow-2xl animate-in fade-in zoom-in-95 duration-500">
            ...
            <p className="text-neutral-400 mb-8 max-w-lg mx-auto">
              Tu encuesta se guardó correctamente. Comparte este enlace con tus
              encuestados.
            </p>
            ...
            <div className="flex flex-col sm:flex-row bg-neutral-950 border border-neutral-800 rounded-xl p-2 max-w-xl mx-auto items-center mb-8 gap-2">
              <input
                ...
                readOnly
                value={createdUrl}
                className="w-full sm:flex-1 bg-transparent px-4 py-2 outline-none text-neutral-300 font-mono text-sm text-center sm:text-left"
              />
              <button
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-4">
              <button
                onClick={handleReset}
                className="px-6 py-3 border border-neutral-700 hover:bg-neutral-800 text-white rounded-xl transition-all font-medium flex-1 sm:flex-none"
              >
                Crear Otra
              </button>
              <Link
                href="/"
                className="px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl transition-all font-medium flex items-center justify-center gap-2 flex-1 sm:flex-none"
              >
          ...
          <>
            <section className="bg-neutral-900 border border-neutral-800 rounded-2xl p-8 shadow-2xl relative overflow-hidden group hover:border-indigo-500/30 transition-colors">
              ...
              {/* El título ahora es un textarea para permitir múltiples líneas */}
              <textarea
                className="w-full bg-transparent text-4xl font-extrabold outline-none placeholder-neutral-600 mb-4 transition-colors resize-none overflow-hidden break-words"
              ...
              {/* A la descripción solo le agregamos 'break-words' */}
              <textarea
                className="w-full bg-transparent text-neutral-400 outline-none placeholder-neutral-700 resize-none overflow-hidden break-words"
            ...
            <section className="bg-neutral-900/50 border border-neutral-800 rounded-2xl p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {categories.map((cat) => (
                  <div
                    key={cat.id}
                    className="flex items-center gap-2 bg-neutral-800 px-3 py-1.5 rounded-lg text-sm border border-neutral-700"
                  >
                    <span>{cat.name}</span>
                    {categories.length > 1 && (
                      <button
            ...
            <section className="space-y-6">
              {questions.map((q, index) => (
                <div
                  key={q.id}
                  className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-xl relative group"
                >
                  ...
                  <div className="flex gap-4 items-start mb-6 pl-2">
                        ...
                        <div className="mt-4 bg-white rounded-xl text-black overflow-hidden border border-neutral-300">
                          <ReactQuill
                            ...
                            onChange={(content: string) =>
                              updateQuestion(q.id, { description: content })
                            }
                    ...
                    <button
                      ...
                      className="text-neutral-500 hover:text-red-400 p-2 hover:bg-neutral-800 rounded-lg transition-colors mt-1"
                    >
                      <Trash2 size={20} />
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6 pl-2">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <select
                        ...
                        value={q.categoryId}
                        onChange={(e) =>
                          updateQuestion(q.id, { categoryId: e.target.value })
                        }
                      >
                        {categories.map((c) => (
                  ...
                  {(q.type === "MULTIPLE_CHOICE" || q.type === "DROPDOWN") && (
                    <div className="pl-2 border-t border-neutral-800/60 pt-4 mt-2">
                      <h4 className="text-xs text-neutral-400 font-semibold mb-3">
                      ...
                      <button
                        ...
                        className="mt-4 flex items-center gap-1 text-sm text-indigo-400 hover:text-indigo-300 font-medium"
                      >
